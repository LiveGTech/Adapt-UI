...
export function getString(size, units = "metric", decimalPlaces = 1) {
    ...
    if (size < radix ** 1) {
        return {unit: "size_bytes", size};
    }
 
    if (size < radix ** 2) {
        return {
            unit: {metric: "size_kb", iec: "size_kib"}[units],
    ...
    if (size < radix ** 3) {
        return {
            unit: {metric: "size_mb", iec: "size_mib"}[units],
    ...
    if (size < radix ** 4) {
        return {
            unit: {metric: "size_gb", iec: "size_gib"}[units],
            size: roundToDecimalPlaces(size / (radix ** 3), decimalPlaces)
        };
    }
 
    return {
        unit: {metric: "size_tb", iec: "size_tib"}[units],
